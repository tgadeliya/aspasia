...
CONSULTANT_PROMPT = strip_prompt("""You will act as an expert consultant arguing for a reading comprehension question. 
    ...
    Structure your output as follows:  <thinking>[THINKING]</thinking>
    <argument>[ARGUMENT]</argument> The judge only sees the output in <argument> tags,
    so focus on clarity and persuasiveness in this section.
""")

CONSULTANT_JUDGE_PROMPT = strip_prompt("""You are an expert judge in a consultancy game 
    ...
    consultant is correct based on your previous
...
DEBATER_JUDGE_PROMPT = strip_prompt("""You are an expert judge in a consultancy game 
    ...
    form without any additional text: 'ANSWER: LETTER', where 'LETTER' is one of the 
    available choices
""")
